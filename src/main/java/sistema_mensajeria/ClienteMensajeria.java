...
public class ClienteMensajeria {
    ...
    private static final String PERSISTENCE_PATH = "src/main/java/sistema_mensajeria/persistence";
    ...
    public ClienteMensajeria() {
        this.server = SERVER;
        this.port = PORT;
        System.out.println("pablito - agosto 1/2024");
    ...
    public ClienteMensajeria(String server, int port) {
        this.server = server;
        this.port = port;
        System.out.println("Carlos E. Gomez - Apr 1/2024");
    ...
    private void saveKeyToFile(String fileName, byte[] key) throws IOException {
        try (FileOutputStream fos = new FileOutputStream(PERSISTENCE_PATH + fileName)) {
            fos.write(key);
        }
    }

    private byte[] readKeyFromFile(String fileName) throws IOException {
        File file = new File(PERSISTENCE_PATH + fileName);
    ...
    private void registerUser(String username) throws Exception {
        ...
        if (keysExist(username)) {
            System.out.println("[Client] el cliente con id: " + username + " ya existe :(. \n registre uno con otro id :)");
            return;
        }
        ...
        saveKeyToFile(username + ".public", publicKey.getEncoded());
        saveKeyToFile(username + ".private", privateKey.getEncoded());
    ...
    private void readMessages(String username) throws Exception {
        ...
        byte[] privateKeyBytes = readKeyFromFile(username + ".private");
    ...
    private boolean keysExist(String username) {
